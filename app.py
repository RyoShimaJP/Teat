...
class RegisterUser(FlaskForm):
    ...
    subscription = SelectField(label='Your subscription type',
                               choices=[('Early Majority - Free', 'Early Majority - Free'),
                                        ('Early Adopter - $100 / mo', 'Early Adopter - $100 / mo'),
                                        ('Innovator - $200 / mo', 'Innovator - $200 / mo')])
...
# Search Index
# wa.whoosh_index(app, Project, User, Tag)
...
def search_results():
    if request.method == "GET":
        ...
        if search_str == "":
            ...
            results2 += Project.query.all()
        else:
            results += User.query.filter(User.name.contains(search_str)).all()
            results2 += Project.query.filter(Project.title.contains(search_str)).all()
        if len(results) == 0 and len(results2) == 0:
            return render_template('search_empty.html')
        return render_template('search_results.html', data=[results, results2])
    else:
